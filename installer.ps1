<#
    ...
    .SYNOPSIS
    This is a simple Powershell script to automate the installation of programs and settings for a fresh install. 
...
if (!$update) {
    ...
    if ($installArray) {
        foreach($installer in $installArray) {
            $count++
            Write-Progress -Activity "Installing..." -PercentComplete ($count / $installArray.Length * 100) -CurrentOperation "$installer"
